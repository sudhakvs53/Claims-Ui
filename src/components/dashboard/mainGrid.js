...
export default class MainGrid extends Component {
    constructor(props) {
        ...
        this.immutableStore = [];
        this.onGridSizeChanged = this
            .onGridSizeChanged
            .bind(this);
        this.getRowNodeId = this
            .getRowNodeId
            .bind(this);
    ...
    rowStyleOptions = (params) => {
        if (params.data.statusType == true) {
            return {background: '#092A4D', color: "#fff"};
        } else {
            return {background: '#144c87', color: "#fff"};
        }
    }
            ...
            <div
                ...
                <div
                    ...
                    <AgGridReact // properties
                        getRowStyle={this.state.getRowStyle} columnDefs={this.state.columnDefs} rowData={this.state.serverData} defaultColDef={this.state.defaultColDef} deltaRowDataMode="true" rowClass={this.state.rowClass} getRowHeight={this.state.getRowHeight} headerHeight={this.state.headerHeight} // rowClassRules             
                        // enableSorting='true'
                        // enableFilter='true'
                        // enableColResize
                        editType="fullRow" enableRangeSelection //rowSelection="multiple"                         
                        // events
                        getRowNodeId={this.getRowNodeId} onGridReady={this.onGridReady} onGridSizeChanged={this.onGridSizeChanged}/>
                </div>
            </div>
